...
void CC1101_Write_Reg( uint8_t Addr, uint8_t WriteValue )
{
	...
	drv_spi_read_write_byte(Addr);
	drv_spi_read_write_byte(WriteValue);
...
{
	uint8_t *pwb = pWriteBuff;
	SPI2_CSN_LOW();
	
	drv_spi_read_write_byte(Addr | WRITE_BURST);
	while(WriteSize--)
	{
		drv_spi_read_write_byte(*pwb);
		pwb++;
	}
...
uint8_t CC1101_Read_Reg( uint8_t Addr )
{
	...
	drv_spi_read_write_byte(Addr | READ_SINGLE );
	revdata = drv_spi_read_write_byte(0xff);
